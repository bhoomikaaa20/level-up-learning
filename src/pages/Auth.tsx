    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-12">
      <Card className="arcade-card w-full max-w-md p-8">
        ...
        <p className="mb-6 text-center text-sm text-muted-foreground">
          {mode === "login" ? "Log in to continue leveling up" : "Earn XP, coins, and badges"}
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === "signup" && (
            <div>
              ...
              <Input id="name" value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="PixelHero" maxLength={40} required />
            </div>
          )}
          <div>
          ...
          <Button type="submit" disabled={submitting} className="w-full bg-gradient-arcade font-semibold text-primary-foreground hover:opacity-90">
            {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : mode === "login" ? "Log in" : "Create account"}
          </Button>
        </form>
