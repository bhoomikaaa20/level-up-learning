...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [{ data: prof }, { data: prog }, { data: ub }] = await Promise.all([
        supabase.from("profiles").select("display_name, xp, coins, streak").eq("id", user.id).maybeSingle(),
        supabase.from("progress").select("level_id, score, xp_earned, played_at, completed, total_questions, correct_count, levels(title)").eq("user_id", user.id).order("played_at", { ascending: false }),
        supabase.from("user_badges").select("badge_id, awarded_at, badge:badges(code, title, icon)").eq("user_id", user.id),
      ]);
      setProfile(prof as Profile);
      const all = prog ?? [];
      setCompleted(all.filter((p: any) => p.completed).length);
      const totalQs = all.reduce((a: number, b: any) => a + b.total_questions, 0);
      const totalCorrect = all.reduce((a: number, b: any) => a + b.correct_count, 0);
      setAccuracy(totalQs > 0 ? Math.round((totalCorrect / totalQs) * 100) : 0);
      setRecent(all.slice(0, 8).map((p: any) => ({
        level_id: p.level_id, score: p.score, xp_earned: p.xp_earned, played_at: p.played_at,
        level_title: p.levels?.title,
      })));
      setBadges((ub ?? []) as any);
      setLoading(false);
    })();
